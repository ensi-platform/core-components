...
export function useCalendar({
    ...
    const focusDay = useCallback(
        ...
        [focusDate, focusFirstAvailableDate, getFocusedDate, maxDate, minDate, offDaysMap, setNextMonth, setPrevMonth]
    );
    ...
    const handleDateRef = useCallback((node: any, index: number) => {
    ...
    const handleDayClick = useCallback(
        (event: MouseEvent<HTMLButtonElement>) => {
            ...
            if (date && onChange) {
                onChange(+date, isKeyboardEvent(event));
            }
        },
        [onChange]
    );
    ...
    const controlsByView = useMemo(
        () => ({
            days: daysControls,
            months: monthControls,
            years: yearsControls,
        }),
        [daysControls, monthControls, yearsControls]
    );
    ...
    const getDayProps = useCallback(
        (day: Day) => {
            ...
            if ((!selected || !isSameMonth(selected, activeMonth)) && !focusableDayIsSet && !day.disabled) {
        ...
        [handleDayClick, handleDateRef, handleKeyDown, handleDayMouseEnter, handleDayMouseLeave, selected]
    );
