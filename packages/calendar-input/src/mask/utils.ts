...
import type { DateSegments, DateTemplate } from '../types';
...
function prepareValueForSegmentation(value: string) {
    if (value.indexOf(' ') !== -1) {
        if (value.indexOf(DATE_RANGE_SEPARATOR) === -1 && value.indexOf(DATE_RANGE_SEPARATOR.trim()) !== -1) {
            return value.replace(/\s/g, '').split(DATE_RANGE_SEPARATOR.trim()).join(DATE_RANGE_SEPARATOR);
        }

        if (value.indexOf(DATE_TIME_SEPARATOR) === -1 && value.indexOf(DATE_TIME_SEPARATOR.trim()) !== -1) {
            return value.replace(/\s/g, '').split(DATE_TIME_SEPARATOR.trim()).join(DATE_TIME_SEPARATOR);
        }
    }
...
export function replaceSegmentsData(
    ...
    const cursorPos = findCursorPlace(rawSegments, template.segments, template.separators, selection);
    ...
    template.segments.forEach((templateSegment, idx, arr) => {
        ...
        if (segment && templateSegment.length < segment.length) {
            const segmentIdx = cursorPos?.beforeNext ? cursorPos.segmentIdx + 1 : cursorPos?.segmentIdx;
...
export function segmentsToObj(segments: string[], templateSegments: string[]): Partial<DateSegments> {
    return {
